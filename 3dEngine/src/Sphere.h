...
public:
	...
	static IndexedTriangleList<V> GetPlain(float radius = 1.0f, int latDiv = 12, int longDiv = 24)
	{
		...
		const float lattitudeAngle = MyMath::PI / latDiv;
		const float longitudeAngle = 2.0f * MyMath::PI / longDiv;

		std::vector<V> vertices;
		for (int iLat = 1; iLat < latDiv; iLat++)
		{
			const auto latBase = base * f_tdMat3::RotationX(lattitudeAngle * iLat);
			for (int iLong = 0; iLong < longDiv; iLong++)
			{
				vertices.emplace_back();
				vertices.back().pos = latBase * f_tdMat3::RotationZ(longitudeAngle * iLong);
			}
		}
