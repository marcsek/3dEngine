...
public:
	...
	constexpr static _Mat ProjectionHFOV(T fov, T ar, T n, T f)
	{
		if constexpr (S == 4)
		{
			const auto fov_rad = fov * (T)MyMath::PI / (T)180.0;
...
template<typename T>
_Vec3<T> operator*(const _Vec3<T>& lhs, const _Mat<T, 4>& rhs)
{
	return{
		lhs.x * rhs.elements[0][0] + lhs.y * rhs.elements[1][0] + lhs.z * rhs.elements[2][0],
		lhs.x * rhs.elements[0][1] + lhs.y * rhs.elements[1][1] + lhs.z * rhs.elements[2][1],
...
typedef _Mat<float, 3> f_tdMat3;
typedef _Mat<double, 3> d_tdMad3;
typedef _Mat<float, 4> f_tdMat4;
typedef _Mat<double, 4> d_tdMad4;